...
class FilesController {
  static async postUpload(req, res) {
    ...
    if (parentId !== "0") {
      const parentFile = await dbClient.dbClient
        .collection("files")
    ...
    const newFile = await dbClient.dbClient
      .collection("files")
      .insertOne({ localPath, ...folderData });
    ...
    return res
      .status(201)
      .json({ id: newFile.insertedId, localPath, ...folderData });
  }
  ...
  static async getIndex(req, res) {
    ...
    const filesCount = await dbClient.dbClient
      .collection("files")
  ...
  static async putPublish(req, res) {
    ...
    await dbClient.dbClient
      .collection("files")
      .updateOne({ _id: ObjectId(fileId) }, { $set: { isPublic: true } });

    const updatedFile = await dbClient.dbClient
      .collection("files")
